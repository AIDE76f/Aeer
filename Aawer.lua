...
-- إنشاء زر Teleport (مبسط وموثوق)
local function CreateTeleportButton()
    -- إذا كان الزر موجود مسبقاً، نحذفه
    if TeleportButton then
        pcall(function() TeleportButton:Destroy() end)
        TeleportButton = nil
    end
    ...
    local success, result = pcall(function()
        ...
        button.Size = UDim2.new(0, 150, 0, 50) -- أكبر قليلاً
        button.Position = UDim2.new(0.5, -75, 0.8, 0)
        button.BackgroundColor3 = Color3.new(0.2, 0.6, 1) -- أزرق فاتح
    ...
    if success and result then
        TeleportButton = result
        
        -- نضيف الزر إلى CoreGui
        pcall(function()
            TeleportButton.Parent = CoreGui
        end)
        
        -- نتأكد أن الأب صحيح
        if TeleportButton.Parent ~= CoreGui then
            TeleportButton.Parent = CoreGui
        end
        
        -- إضافة وظيفة النقر
        TeleportButton.MouseButton1Click:Connect(function()
            print("Teleport button clicked!") -- للتأكد من أن الزر يعمل
        ...
        print("✅ Teleport button created successfully!")
    else
        warn("❌ Failed to create teleport button:", result)
    end
end
...
ScreenGui.ResetOnSpawn = false -- مهم جداً لمنع اختفاء الواجهة عند الموت
...
do
    ...
    y = CreateToggle(ComboContent, "Teleport Player", function() return Settings.Combo.TeleportPlayer.Enabled end, function(v)
        ...
        print("Teleport toggled:", v) -- للتأكد من أن التبديل يعمل
        ...
        if v then
            ...
            if TeleportButton then
                TeleportButton:Destroy()
                TeleportButton = nil
                print("Teleport button destroyed")
            end
        end
    end, y)
...
print("✅ Script loaded successfully! Teleport button will appear when enabled.")

-- نتحقق من حالة Teleport بعد تحميل السكربت
task.wait(1)
if Settings.Combo.TeleportPlayer.Enabled then
    print("Teleport was enabled, creating button...")
    CreateTeleportButton()
end
